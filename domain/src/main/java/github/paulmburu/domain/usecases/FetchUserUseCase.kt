...
typealias FetchUserBaseUseCase = FlowBaseUseCase<String, Flow<Resource<User>>>

class FetchUserUseCase constructor(private val githubRepository: GithubRepository) :
    FetchUserBaseUseCase {
    override suspend fun invoke(params: String): Flow<Resource<User>> = flow{
        val result = githubRepository.fetchUser(params)
